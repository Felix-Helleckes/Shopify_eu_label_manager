# Felix: deine Liste (Stand 11.09.2026)

Nur die Dinge, die wirklich deine Accounts, deine Zahlung oder deine Stimme brauchen. Alles andere mache ich.

## Heute (zusammen 30 Minuten)

1. ~~**Fly.io einrichten**~~ — **erledigt am 11.09.2026.** Die App läuft in Frankfurt unter
   https://eu-compliance-suite.fly.dev, Kosten ca. 3,30 USD/Monat. Eine eigene Domain ist nicht nötig.
2. ~~**Einreichung**~~ — **erledigt am 11.09.2026.** Status: „Eingereicht, Prüfer wird zugewiesen“. Rückmeldungen
   kommen per E-Mail an florian.helleckes@gmail.com — **täglich prüfen und sofort antworten**, sonst stoppt die Prüfung.
3. **Agenturen anschreiben** (20 Min): 18 fertige Nachrichten in `outreach-messages.md`, je Kontaktformular oder
   LinkedIn. Reihenfolge ist Priorität; die ersten 8 reichen für heute.

## Am Tag der Freigabe (30 Minuten)

4. LinkedIn-Post DE + EN aus `posts.md`, Bild: `store/listing/en/screenshot-cover.png`.
5. Shopify Community: je ein Thread DE + EN aus `posts.md`; Reddit r/shopify und r/ecommerce_de (Text dort).
6. Mir den App-Store-Link schicken – ich setze ihn in Landingpage, Listing (Tutorial-Feld) und lege das englische
   Listing an.

## In der ersten Woche

7. **Zwei YouTube-Videos** über theefficientdev nach `youtube-howto.md` (EN + DE, je 5 Minuten Bildschirmaufnahme).
   Das ist der einzige Content-Kanal, der über Suche dauerhaft Händler bringt.
8. **Bewertungen:** jeden Shop, der die App installiert, persönlich anschreiben (Vorlage unten). Zehn Bewertungen
   entscheiden das Ranking.

Vorlage Bewertungsbitte (DE):
> Hallo [Name], danke, dass Sie die EU Compliance Suite installiert haben. Wenn alles läuft: eine kurze Bewertung im
> App Store hilft mir als Ein-Personen-Anbieter enorm (https://apps.shopify.com/eu-label-1/reviews). Wenn etwas hakt:
> einfach auf diese Mail antworten, ich kümmere mich am selben Tag. Beste Grüße, Felix

## Optional, wenn Geld übrig ist

9. Netlify Pro (19 $/Monat) – nur nötig, wenn du vor dem 23.09. die Haupt-URL statt der Alias-URL willst.
10. Shopify App Store Ads, 5–10 €/Tag auf „GARAN“, „Gewährleistungslabel“, „withdrawal button“ in den zwei Wochen vor
    dem 27.09. Der einzige bezahlte Kanal, den ich empfehle.

## Was ich weiter übernehme
Domain-Einrichtung, englisches Listing, App-Store-Link einbauen, Support-Antworten vorbereiten, Bugfixes, Umstellung auf
die Haupt-URL nach dem 23.09., Auswertung der ersten Installationen.
